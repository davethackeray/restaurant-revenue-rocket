...
- **Tasks**:
  1. **Week 7, Day 1-2**: Review existing frontend codebase (React) to identify integration points for displaying AI-driven scenarios, focusing on scenario rendering components.
  ...
  4. **Week 8, Day 1-3**: Extend demo profile JSON files to include agentic AI behavior parameters tailored to restaurant types (e.g., fine dining AI prioritizes customer experience with higher weight on satisfaction metrics, quick service prioritizes speed with faster decision cycles).
  5. **Week 8, Day 4-5**: Update profile loading logic in backend to parse and apply AI behavior parameters during scenario initialization.
